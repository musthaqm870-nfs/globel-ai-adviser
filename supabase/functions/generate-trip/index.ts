...
serve(async (req) => {
  ...
  try {
    ...
    if (totalRequests >= 10) {
      ...
      return new Response(
        JSON.stringify({ error: "Rate limit exceeded. You can generate up to 10 trips per hour. Please try again later." }),
        {
          status: 429,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        }
    ...
    await supabase
      .from('api_usage')
      .insert({ user_id: user.id, endpoint: 'generate-trip', request_count: 1 });
